...
const ApplicationCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            ApplicationName: _entity?.ApplicationName,
Language: _entity?.Language,
TimeZone: _entity?.TimeZone,
AppPerformance: _entity?.AppPerformance,
        };

        setLoading(true);
        try {
            
        const result = await client.service("application").patch(_entity._id, _data);
        props.onHide();
    ...
    return (
        <Dialog header="Edit Application" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="application-edit-dialog-component">
                <div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            <label htmlFor="ApplicationName">ApplicationName:</label>
            <InputText id="ApplicationName" className="w-full mb-3 p-inputtext-sm" value={_entity?.ApplicationName} onChange={(e) => setValByKey("ApplicationName", e.target.value)}  />
        </span>
        </div>
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
...
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            ...
            <Calendar id="TimeZone" value={_entity?.TimeZone ? new Date(_entity?.TimeZone) : new Date()} onChange={ (e) => setValByKey("TimeZone", e.value)} showTime hourFormat="24"  />
        </span>
        </div>
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            <label htmlFor="AppPerformance">AppPerformance:</label>
            <InputNumber id="AppPerformance" className="w-full mb-3 p-inputtext-sm" value={_entity?.AppPerformance} onChange={(e) => setValByKey("AppPerformance", e.target.value)}  />
        </span>
        </div>
