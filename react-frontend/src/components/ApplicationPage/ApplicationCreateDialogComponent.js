...
const ApplicationCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            ApplicationName: _entity?.ApplicationName,Language: _entity?.Language,TimeZone: _entity?.TimeZone,AppPerformance: _entity?.AppPerformance,
    ...
    return (
        <Dialog header="Create Application" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            {!_.isEmpty(error["ApplicationName"]) ? (
              <p className="m-0" key="error-ApplicationName">
...
<div className="col-12 md:col-6 field mt-5">
            ...
            <small className="p-error">
            {!_.isEmpty(error["AppPerformance"]) ? (
              <p className="m-0" key="error-AppPerformance">
                {error["AppPerformance"]}
              </p>
            ) : null}
          </small>
            </div>
